...
- 内存堆栈存储分为：大端【内存地址左到右读取（从低位到高位存储）】、小端【内存地址右到左读取（从高位到低位存储【左到右】）】存储
# 二、数据类型
* <span style="color:red;font-weight:800">标识符：字母+数字+下划线；开头字符必须为字母或下划线；不能是关键字
* <span style="color:red;font-weight:800">变量区分大小写</span>
* <span style="color:red;font-weight:800">小数不能取模%</span>
* <span style="color:red;font-weight:800">非0数表述逻辑真</span>
## 2.0标识符
- 只能由字母数字下划线组成，开头字符必须为字母或下划线
## 2.1 基本数据类型
1. 整型 int short long
2. 字符型 char
3. 实型（浮点型）
